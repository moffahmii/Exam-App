    ...
    if (status === 'error') {
        return (
            <div className="text-center p-20 font-mono text-red-500 bg-red-50 rounded-xl border border-red-100">
                ⚠️ Failed to load diplomas. Please refresh the page.
            </div>
        )
    }
    return (
        <section className="w-full space-y-12">
            ...
            <div ref={ref} className="py-10 flex flex-col items-center gap-2">
                {isFetchingNextPage ? (
                    <div className="flex items-center gap-2 font-mono text-blue-600 animate-pulse">
                        <div className="w-2 h-2 bg-blue-600 rounded-full animate-bounce" />
                        Loading more...
                    </div>
                ) : hasNextPage ? (
                    <span className="font-mono text-gray-400 text-sm italic">
                        Scroll down to discover more ↓
                    </span>
                ) : (
                    <div className="h-px w-full bg-gray-100 relative">
                        <span className="absolute left-1/2 -translate-x-1/2 -top-3 bg-white px-4 font-mono text-xs text-gray-300">
                            END OF DIPLOMAS
                        </span>
                    </div>
                )}
