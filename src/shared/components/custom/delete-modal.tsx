...
export function GlobalDeleteModal({
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-200">
            {/* العرض 549px (w-139.5) والطول 412px (h-103) كما طلبت بالحرف */}
            <div className="relative bg-white w-139.5 h-103 flex flex-col animate-in duration-200 font-mono">
                ...
                <div className="flex-1 p-9 flex flex-col items-center text-center justify-center">
                    ...
                    <div className="text-sm text-gray-500">
                        <p className="text-red-600 text-lg font-medium mb-2 leading-tight">
                            {title}
                        </p>
                        {description}
                    </div>
                ...
                {/* منطقة الأزرار (الـ Footer) بنفس تنسيق الـ 50/50 والثوابت الخاصة بك */}
                <div className="border-t border-gray-200 bg-gray-50 flex items-center justify-center gap-4 py-4 px-13.5">
                    <Button
                        variant="ghost"
                        onClick={onClose}
                        className="flex-1 h-12 w-54.25 text-gray-800 bg-gray-200 hover:bg-gray-400 text-sm font-medium border border-gray-200 rounded-none"
                    >
                        Cancel
                    </Button>
                    ...
                    <Button
                        disabled={isLoading}
                        onClick={onConfirm}
                        className="flex-1 h-12 w-54.25 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-none"
                    >
                        {isLoading ? (
